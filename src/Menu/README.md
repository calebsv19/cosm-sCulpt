...
- `line_drawing_scene_catalog.h` / `line_drawing_scene_catalog.c` — first app-local catalog backend for Phase 2. It scans the input root for `.json` layouts and valid authored scene directories (`scene_authoring.json` + `scene_runtime.json`), sorts entries, and tracks which layout/scene path is currently active.

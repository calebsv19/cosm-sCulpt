...
- `ui_panel_right_controls.h` / `panel/ui_panel_right_controls.c` — own the shared right-pane control-grid lane. They define the compact row variants for `View`, `Create`/object-mode `Tools`, and `Object`/object-mode `Properties`, derive section height from the same unique sorted visible-row inventory used for placement, and lay out lower clusters as uniform equal-width rows independent of button registration order.
...
- `ui_panel_scene_authoring_inspector.h` / `panel/ui_panel_scene_authoring_inspector.c` — own camera/light/path authoring readback and mutations. Path selection exposes play/pause, normalized scrub, duration presets, once/loop policy, and open/closed state; the summary reports evaluated route length and current progress. These controls mutate the loaded authoring document through the normal history/dirty-state lane.
...
- `panel/ui_panel_dialog_logic.c` — typed dialog apply/cancel logic for dimensions, bounds, offsets, and selected-object transform editing. The selected-object dimension dialog edits plane/prism width and height through the same exact controls, while depth remains rect-prism-only.
- `panel/ui_panel_file_ops.c` — save/load/export and root-path file-operation helpers. It owns the JSON/scene root picker call sites and `Export Shape` / `Export Scene`; scene export writes `scene_authoring.json` plus compiled `scene_runtime.json` under the configured output root, reports the export result in the File summary status line, and promotes the exported authoring file as the active/recent scene session. Active scene paths are used only as naming hints during export, not as implicit overwrite destinations. Session input-root edits now stay explicitly separate from the lower file-browser mode roots and only rebuild the browser when the browser is actually following the same session root.
- `platform/line_drawing_folder_picker.h` / `platform/line_drawing_folder_picker.c` — app-local native folder-picker adapter. macOS uses `osascript`; Linux tries `zenity` then `kdialog` through argument-vector process execution. If neither picker is usable, root state remains unchanged and the existing manual path-entry controls remain available.
...
- `topbar/line_drawing_editor_topbar.h` / `topbar/line_drawing_editor_topbar.c` — own the production top-level menu/status bar. It renders one coordinated top-pane surface with the `Scene` / `Object` workspace switch, clipped selected-object chip, selection context, file/dirty state, mode, view, plane, construction-plane, bounds, gizmo mode, live operation, and undo/redo groups so the top lane no longer stacks independent overlay renderers. `Mode`, `View`, `Plane`, `Bounds`, `Gizmo`, `Undo`, and `Redo` are active status chips: `Mode` mirrors `M`, `View` mirrors `V` and can enter 3D/free-view from 2D, `Plane` cycles the active edit plane, `Bounds` toggles scene bounds through the panel bounds helper, `Gizmo` mirrors `X`, and `Undo`/`Redo` mirror the editor history shortcuts; during active center-gizmo drags, the separate `Op` chip and primary selection line report live move distance, rotate angle, or size factor. `CP` remains a readout until its picker/stepper workflow is explicit.
...
- The shell polish pass now adds stronger visual hierarchy without changing the
  ...
  - the file pane now uses the same owned-section approach instead of the old
    ...
    - `File / IO` and `Session Paths` are compact anchored bottom groups
      - both bottom groups now use equal-width 2-column button rows where appropriate
      - export actions surface success/failure status in the summary instead of relying only on log output
    ...
    - browser row loads now expand the selected path with a compact progress
      lane for JSON, scene, object asset, runtime mesh, and STL import actions;
      JSON/scene/object/runtime loads report queued/loading plus a final
      completion/failure state, completed lanes auto-collapse, and STL import
      now runs the heavy import conversion on a background SDL thread with
      staged compiler progress before placing the generated runtime mesh on
      the main thread; fresh STL runtime caches are reused as immediate
      `Add` placements while stale source files are reimported

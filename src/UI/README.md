...
- `font_manager.h` / `font_manager.c` — initialise SDL_ttf, load font resources from `include/fonts`, expose typed getters, bridge shared font preset selection into runtime reloads, and release fonts on shutdown.
...
- Rendering runs `Render_InfoOverlay` (status bar) followed by pane-clipped `Render_UIPanelSide(...)` / `Render_UIPanelRootSummary(...)` work, `UIPanel_RenderOverlays` for dropdown/save/root modals, and the active-only Workspace Authoring overlay. Pane mode overlays the program; Font/Theme mode fills the viewport and previews runtime font/theme changes.

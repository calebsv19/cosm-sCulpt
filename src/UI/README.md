...
- `input_ui_panel.h` / `input_ui_panel.c` — detect clicks inside button bounds before layout editing occurs. Executes actions such as saving/loading JSON (now via a modal filename prompt and dropdown), resetting the origin, zooming, toggling delete mode, pinning the selected anchor, and toggling `SpaceMode` (`2D`/`3D`), and raises the appropriate dirty flags so hitboxes/layout stay in sync.

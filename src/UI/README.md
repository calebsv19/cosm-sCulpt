...
- `input_ui_panel.h` / `input_ui_panel.c` — detect clicks inside button bounds before layout editing occurs. Executes actions such as saving/loading JSON, resetting the origin, zooming, toggling delete mode, and pinning the selected anchor, and raises the appropriate dirty flags so hitboxes/layout stay in sync.
- `info_overlay.h` / `info_overlay.c` — render a top-of-screen status bar displaying details about the current selection, delete mode, and undo/redo shortcuts.
...
- Rendering runs `Render_InfoOverlay` (status bar) followed by `Render_UIPanel` so contextual data appears first and action buttons remain visible just beneath it.

...
- Rendering runs `Render_InfoOverlay` (status bar) followed by pane-clipped `Render_UIPanelSide(...)` / `Render_UIPanelRootSummary(...)` work, `UIPanel_RenderOverlays` for dropdown/save/root modals, and the active-only Workspace Authoring pane overlay when authoring pane mode is selected.

...
typedef struct SDLAppLoopDiagState {
    bool initialized;
    bool enabled;
    bool json_output;
...
static SDLAppLoopDiagState s_loop_diag = {0};

static bool sdl_app_loop_diag_env_truthy(const char* value) {
    return value && value[0] &&
           (strcmp(value, "1") == 0 || strcasecmp(value, "true") == 0);
}

static void sdl_app_loop_diag_init_once(void) {
    if (s_loop_diag.initialized) {
        return;
    }
    if (sdl_app_loop_diag_env_truthy(getenv("LINE_DRAWING_LOOP_DIAG_LOG"))) {
        s_loop_diag.enabled = true;
    }
    {
        const char* format_env = getenv("LINE_DRAWING_LOOP_DIAG_FORMAT");
        if (format_env && format_env[0] && strcasecmp(format_env, "json") == 0) {
            s_loop_diag.enabled = true;
            s_loop_diag.json_output = true;
        }
    }
    if (sdl_app_loop_diag_env_truthy(getenv("LINE_DRAWING_LOOP_DIAG_JSON"))) {
        s_loop_diag.enabled = true;
        s_loop_diag.json_output = true;
    }
...
static void sdl_app_loop_diag_emit(uint64_t period_ms) {
...
void SDLAppLoop_DiagTick(double frame_elapsed_sec,
    ...
    sdl_app_loop_diag_emit(s_loop_diag.period_elapsed_ms);

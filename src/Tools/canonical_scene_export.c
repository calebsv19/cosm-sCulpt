...
static cJSON* build_scene_json(const Layout* layout,
    ...
    cJSON_AddStringToObject(root, "scene_id", string_or_default(scene_id, kDefaultSceneId));
...

    {
        ...
        cJSON_AddStringToObject(material, "material_id", kDefaultMaterialId);
        cJSON_AddStringToObject(material, "material_type", "flat_color");
...

    {
        ...
        cJSON_AddStringToObject(light, "light_id", kDefaultLightId);
        cJSON_AddStringToObject(light, "light_type", "directional");
...

    {
        ...
        cJSON_AddStringToObject(camera, "camera_id", kDefaultCameraId);
        cJSON_AddStringToObject(camera, "camera_type", is_3d ? "perspective" : "orthographic");
...
char* LineDrawingCanonicalScene_ExportLayoutToString(const Layout* layout,
                                                     const char* sceneId) {
    cJSON* root = build_scene_json(layout, sceneId, NULL);
    char* out = NULL;
    if (!root) return NULL;
...
bool LineDrawingCanonicalScene_ExportLayoutToFile(const Layout* layout,
    ...
    root = build_scene_json(layout, sceneId, existing_root);
